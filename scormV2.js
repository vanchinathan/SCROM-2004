...
// Helpers:
var scormPost = function (send_values) {
  parent.postMessage({ sendValues: send_values }, '*');
};
...
function LMSGetValue(parameter) {
  if (_DEBUG)
    console.log("Entering LMSGetValue, argument: '" + parameter + "'");

  myInitError();
  return_value = myGetValues(parameter);

  if (_DEBUG)
    console.log("Exiting LMSGetValue, returns: '" + return_value + "'");
...
function LMSSetValue(parameter, value) {
  ...
  if (_DEBUG)
    console.log("Exiting LMSSetValue, returns: '" + return_value + "'");
  return return_value;
}
...
function myCommit(parameter) {
  ...
  if (parameter != '' && parameter != 'finish') {
    //'finish' parameter indicates that myCommit was called from myFinish, so it may persist total_time as well
    ...
    try {
      ...
      for (var i = 0; i < cmi.interactions.length; i++) {
        ...
        for (var j = 0; j < cmi.interactions[i].correct_responses.length; j++) {
          SCOState.interactions[i].correct_responses[j] = {
            pattern: cmi.interactions[i].correct_responses[j].pattern.value,
          };
        }
      }

      commitState = { ...SCOState }; //make a copy of the object, so we can unset the session_time if we're not finished
      if (parameter != 'finish') {
      ...
      if (
        ...
        typeof JSInterface.commit != 'undefined'
      ) {
...
function myGetErrorString(errorNumber) {
  ...
  if (errorNumber == '') {
    //No arguments were given, so no error is reported (this is basically to comply with the case where the function is called before LMSInitialize())
    return '';
  } else if (typeof errorStrings[errorNumber] == 'undefined') {
    //If the error code is not valid, do nothing really
...
function myGetDiagnostic(errorNumber) {
  ...
  errorDiagnostic['202'] =
    'This element cannot have children - Element cannot have children';
  errorDiagnostic['203'] =
    'This element is not an array - Element not an array - cannot have count';
  errorDiagnostic['301'] = 'System has to be initialized - Not initialized';
  errorDiagnostic['401'] =
    'This property is not implemented - Not implemented error';
  errorDiagnostic['402'] =
    'You cannot set a value to a keyword - Invalid set value, element is a keyword';
  errorDiagnostic['403'] =
    "You can only read this element's value - Element is read only";
  errorDiagnostic['404'] =
    "You can only write this element's value - Element is write only";
  errorDiagnostic['405'] =
    'You cannot set this element to this value - Incorrect Data Type';
...
function checkDataType(parameter, data_model, type) {
  switch (data_model) {
    ...
    case 'CMIFeedback':
      switch (type) {
        ...
        case 'likert':
          ...
          break;
        case 'matching':
          return (match = /^[a-z0-9].[a-z0-9](,[a-z0-9].[a-z0-9])*$/i.test(
            parameter,
          )); //Pair of identifiers, i.e. 2.s,4.2
          break;
        case 'performance':
    ...
    case 'CMISInteger':
      return (match =
        /^(\-|\+)?[0-9]{1,5}$/i.test(parameter) &&
        parameter <= 32768 &&
        parameter >= -32768); //Signed integer ranging from -32768 to +32768
      break;
    ...
    case 'CMIString4096':
      ...
      return (match = /^.{0,65535}$/im.test(parameter)); //By violation of the protocol, we set this to SCORM 2004's limit
      break;
    ...
    case 'CMIVocabulary':
      switch (type) {
        case 'Mode':
          ...
          break;
        case 'Status':
          return (match = /^(passed|completed|failed|incomplete|browsed|not attempted)$/.test(
            parameter,
          ));
          break;
        case 'Exit':
        ...
        case 'Entry':
          ...
          break;
        case 'Interaction':
          return (match = /^(true-false|choice|fill-in|matching|performance|likert|sequencing|numeric)$/.test(
            parameter,
          ));
          break;
        case 'Result':
          return (match = /^(correct|wrong|unanticipated|neutral|(-?\d+(\.\d+)?))$/.test(
            parameter,
          ));
          break;
        case 'TimeLimitAction':
          return (match = /^(exit,message|exit,no message|continue,message|continue,no message)$/.test(
            parameter,
          ));
          break;
        default:
...
function myCmi() {
  ...
  this.core = new (function () {
    ...
    var lesson_status = function () {
      ...
      this.set = function (param) {
        ...
        if (
          !checkDataType(param, 'CMIVocabulary', 'Status') ||
          param == 'not attempted'
        ) {
          throw new myError('405');
        } else {
    ...
    var entry = function () {
      ...
      this.get = function () {
        if (typeof SCOState.entry != 'undefined') {
          //Since entry may be just '', we need to check if it is defined
    ...
    var score = function () {
      ...
      var raw = function () {
        ...
        this.set = function (param) {
          if (
            (!checkDataType(param, 'CMIDecimal', false) ||
              param < 0 ||
              param > 100) &&
            !checkDataType(param, 'CMIBlank', false)
          ) {
            throw new myError('405');
          } else {
            this.value = param;
  ...
  this.student_preference = new (function () {
    ...
    var audio = function () {
      ...
      this.set = function (param) {
        if (
          !checkDataType(param, 'CMISInteger', false) ||
          param < -1 ||
          param > 100
        ) {
          throw new myError('405');
...
objectivesObject = function () {
  ...
  this.id = new (function () {
    this.get = function () {
      if (this.value === null) {
        //means that is not initialized yet
  ...
  this.status = new (function () {
    ...
    this.set = function (param) {
      ...
      if (!checkDataType(param, 'CMIVocabulary', 'Status')) {
        ...
        this.value = param;
        if (SCOState.lesson_status == 'not attempted') {
          //meaning that the SCO doesn't manipulate the lesson_status parameter at all
          SCOState.lesson_status = this.value;
        }
        return 'true';
      }
    ...
    this.set = function (param) {
      if (
        !checkDataType(
          param,
          'CMIFeedback',
          cmi.interactions[_TEMP].type.getValue(),
        )
      ) {
        throw new myError('405');
      } else {
